...
a package from bunch of files.

You were given two so called source packages - these are packages from which
the REAL packages are build from and end with '.src.rpm'. To look inside such
...
our package. To build a package from that recipe, we can use rpmbuild command:
...
you a opportunity to fix the problem a try to build it again. The problem can
